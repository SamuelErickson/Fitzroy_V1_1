...
def sunrise(arg1,arg2,arg3,arg4):
    """
    Example:
    ...
    argument = str(arg1)+" "+str(arg2)+" "+str(arg3)+" "+str(arg4)
    os.system("screen -d -m -S sunrise bash -c \"python3 sunrise.py "+argument+"\"")

def sunset(arg1,arg2,arg3,arg4):
    """
    Example:
    python3 sunrisesunset_24h.py 18 0.75 10 500
    means drive light with raspberry pi pin 18, maximum duty cycle 0.75, sunrise/sunset duration 10 seconds,
    oscillation frequency 500 hz, on at 8:30 AM, off at 8:45 PM"""
    argument = str(arg1)+" "+str(arg2)+" "+str(arg3)+" "+str(arg4)
    os.system("screen -d -m -S sunset bash -c \"python3 sunset.py "+argument+"\"")
...
def retrieve_update_values():
    ...
    box configuration from config.csv. Updates settings.csv with arguments passed to this script.
    ...
    #open the config.csv and create pandas dataframe from which to call values
    ...
    parameters['LogInterval_sec'] = int(df_settings["LogInterval_sec"])
    parameters["DisplayWindow_sec"] = int(df_settings["DisplayWindow_sec"])
    ...
    #update the settings.csv file
    df_settings['TempSetPoint'] = parameters["tempSetPoint"]
    #df_settings['Settings'] = parameters["tempSetPoint"]
...
def initializeIO(parameters,vals):
    ...
    vals["FanPower"] = parameters["fanDC"]
    vals["LightStatus"] = "OFF"
    return pi, s, vals
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            #check whether time for sunrise or sunset
            timeNow = vals["Time"].time()
            isDayTime = (timeNow > parameters["sunriseTime"] and timeNow < parameters["sunriseTime"])
            if isDayTime and vals["LightStatus"] == "OFF":
                vals["LightStatus"] = "ON"
                sunrise(parameters["light_pin"],parameters["light_pin"],parameters["light_pin"],parameters["light_pwm_freq_hz"])
            elif not isDayTime and vals["LightStatus"] == "ON":
                vals["LightStatus"] = "OFF"
                sunset(parameters["light_pin"],parameters["light_pin"],parameters["light_pin"],parameters["light_pwm_freq_hz"])
            #query sensor
            if not runningOnPC:
                temp, humidity = query_DHT(s)
            else:
